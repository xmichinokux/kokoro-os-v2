...
import { buildShortResponse } from '@/lib/kokoro/buildShortResponse';
...
export default function KokoroCoreView({ data }: Props) {
  ...
  const shortResponse = buildShortResponse({
    deepFeeling: data.convergence.trueFeeling,
    conflictAxes: data.conflict?.axes,
    personas: data.personas,
    action: data.convergence.action,
  });

  // 一言が空の場合はデフォルト展開
  const [expanded, setExpanded] = useState(!shortResponse);
  ...
  return (
    <div style={{ maxWidth: '100%' }}>
      ...
      {/* 第1層：一言（常に表示） */}
      {shortResponse && (
        <div style={{ fontSize: 14, lineHeight: 1.9, color: '#374151', fontWeight: 400, marginBottom: 12 }}>
          {shortResponse}
        </div>
      )}
      ...
      <div style={{ marginBottom: expanded ? 20 : 0 }}>
        <button
          ...
          style={{
            ...
            color: expanded ? '#9ca3af' : '#7c3aed',
            background: expanded ? 'transparent' : '#ede9fe',
            border: `1px solid ${expanded ? '#e5e7eb' : '#c4b5fd'}`,
            borderRadius: 4,
            padding: '5px 14px',
            cursor: 'pointer',
